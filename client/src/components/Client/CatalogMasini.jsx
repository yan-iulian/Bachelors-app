...
// Read persisted AI match scores per user (localStorage)
const getAiMatchScores = (userId) => {
  try {
...
const matchColor = (pct) => {
  ...
  if (pct >= 60)
    return { text: "text-teal-400", hex: "#2dd4bf", ring: "ring-teal-400/40" };
  if (pct >= 45)
    return {
...
const getSpecs = (m) =>
  `${combustibilMap[m.combustibil] || "—"} • ${m.anFabricatie} • ${formatKm(m.km)}`;
const getImagine = (m) =>
  m.imaginePrincipala
...
function CatalogMasini() {
  ...
  const [sortBy, setSortBy] = useState(
    () => sessionStorage.getItem("catalogSort") || "newest",
  );
  const [paginaCurenta, setPaginaCurenta] = useState(
    () => Number(sessionStorage.getItem("catalogPage")) || 1,
  );
  const [aiScores, setAiScores] = useState(() => getAiMatchScores(userId));
  ...
  const [selectedBrands, setSelectedBrands] = useState(() => {
    try {
      return JSON.parse(sessionStorage.getItem("catalogBrands") || "[]");
    } catch {
      return [];
    }
  });
  const [fuelFilter, setFuelFilter] = useState(
    () => sessionStorage.getItem("catalogFuel") || null,
  );
  const [selectedCaroserii, setSelectedCaroserii] = useState(() => {
    try {
      return JSON.parse(sessionStorage.getItem("catalogCaroserii") || "[]");
    } catch {
      return [];
    }
  });
  ...
  useEffect(() => {
    sessionStorage.setItem("catalogPage", paginaCurenta);
  }, [paginaCurenta]);
  useEffect(() => {
    sessionStorage.setItem("catalogBrands", JSON.stringify(selectedBrands));
  }, [selectedBrands]);
  useEffect(() => {
    sessionStorage.setItem("catalogFuel", fuelFilter || "");
  }, [fuelFilter]);
  useEffect(() => {
    sessionStorage.setItem(
      "catalogCaroserii",
      JSON.stringify(selectedCaroserii),
    );
  }, [selectedCaroserii]);
  ...
  useEffect(() => {
    const fetchMasini = async () => {
      try {
        ...
        const scores = getAiMatchScores(userId);
        setAiScores(scores);
            ...
            if (d.status === "Approved" && d.tip === "Discount" && d.idMasina) {
              ...
              if (masina) {
                discountMap[d.idMasina] = {
                  ...
                  pretPersonal: Math.round(
                    masina.pretEuro * (1 - d.discountProcent / 100),
                  ),
                };
              }
            }
  ...
  useEffect(() => {
    const handleFocus = () => {
      const fresh = getAiMatchScores(userId);
  ...
  const ABS_MIN_YEAR =
    masini.length > 0 ? Math.min(...masini.map((m) => m.anFabricatie)) : 2000;
  const ABS_MAX_YEAR =
    masini.length > 0 ? Math.max(...masini.map((m) => m.anFabricatie)) : 2025;
  const ABS_MIN_PRICE =
    masini.length > 0
      ? Math.floor(
          Math.min(
  ...
  const ABS_MAX_PRICE =
    masini.length > 0
      ? Math.ceil(Math.max(...masini.map((m) => m.pretEuro)) / 10000) * 10000
      : 200000;
  ...
  const filteredMasini = masini.filter((m) => {
    ...
    if (selectedBrands.length > 0 && !selectedBrands.includes(m.marca))
      return false;
    if (
      selectedCaroserii.length > 0 &&
      !selectedCaroserii.includes(m.categorieAuto)
    )
      return false;
    if (fuelFilter && combustibilMap[m.combustibil] !== fuelFilter)
      return false;
  ...
  const sortedMasini = [...filteredMasini].sort((a, b) => {
    ...
    if (sortBy === "price-low")
      return (
        (a.esteInPromotie ? a.pretPromotional : a.pretEuro) -
        (b.esteInPromotie ? b.pretPromotional : b.pretEuro)
      );
  ...
  const paginatedMasini = sortedMasini.slice(
    (paginaCurenta - 1) * ITEMS_PER_PAGE,
    paginaCurenta * ITEMS_PER_PAGE,
  );
  ...
  // Slider percentages
  const yearMinPct =
    ABS_MAX_YEAR > ABS_MIN_YEAR
      ? ((yearMin - ABS_MIN_YEAR) / (ABS_MAX_YEAR - ABS_MIN_YEAR)) * 100
      : 0;
  const yearMaxPct =
    ABS_MAX_YEAR > ABS_MIN_YEAR
      ? ((yearMax - ABS_MIN_YEAR) / (ABS_MAX_YEAR - ABS_MIN_YEAR)) * 100
      : 100;
  const priceMinPct =
    ABS_MAX_PRICE > ABS_MIN_PRICE
      ? ((priceMin - ABS_MIN_PRICE) / (ABS_MAX_PRICE - ABS_MIN_PRICE)) * 100
      : 0;
  const priceMaxPct =
    ABS_MAX_PRICE > ABS_MIN_PRICE
      ? ((priceMax - ABS_MIN_PRICE) / (ABS_MAX_PRICE - ABS_MIN_PRICE)) * 100
      : 100;
      ...
      <aside className="hidden lg:flex flex-col w-80 shrink-0 gap-6 sticky top-24 h-[calc(100vh-8rem)] overflow-y-auto pr-2 pb-10">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-xs font-medium text-[#895af6] hover:text-white transition-colors"
          >
            Resetează
          </button>
        </div>
        ...
        <div className="glass-panel p-5 rounded-2xl space-y-4">
          ...
          <div>
            ...
            <div className="relative h-1.5 bg-white/10 rounded-full">
              <div
                className="absolute h-full bg-[#895af6] rounded-full"
                style={{ width: `${priceMinPct}%` }}
              ></div>
              <input
                type="range"
                min={ABS_MIN_PRICE}
              ...
              />
              <div
                className="absolute size-4 bg-[#895af6] border-2 border-white rounded-full -top-1.5 shadow-lg pointer-events-none"
                style={{
                  left: `${priceMinPct}%`,
            ...
            {[
              {
                id: 0,
                name: "Sedan",
                path: "M5 11l2-3h10l2 3v6h-2v-1H7v1H5v-6zM3 11v6h2v-6H3zm18 0v6h-2v-6h2zM7 14h2v2H7v-2zm8 0h2v2h-2v-2z",
              },
              {
                id: 1,
                name: "Hatchback",
                path: "M4 11l2-4h8l4 4v6h-2v-1H6v1H4v-6zm2 2v2h2v-2H6zm10 0v2h2v-2h-2zm-9-4l-1.5 2h13l-2.5-2H7z",
              },
              {
                id: 2,
                name: "Coupe",
                path: "M3 12l2.5-3.5h13L21 12v6h-2v-1H5v1H3v-6zm3.5-2l-1.5 2h14l-1.5-2H6.5zM7 14h2v2H7v-2zm8 0h2v2h-2v-2z",
              },
              {
                id: 3,
                name: "Break",
                path: "M4 11l1-3h13l2 3v6h-2v-1H6v1H4v-6zm2 2v2h2v-2H6zm10 0v2h2v-2h-2zm-9-4l-.7 2h11.4l.7-2H7z",
              },
              {
                id: 4,
                name: "SUV",
                path: "M4 10l1.5-4h13L20 10v7h-2v-1H6v1H4v-7zm3 4h2v2H7v-2zm8 0h2v2h-2v-2zm-8.5-4l-1 2.5h12l-1-2.5H6.5z",
              },
              {
                id: 5,
                name: "Cabrio",
                path: "M3 12l2-3h14l2 3v5h-2v-1H5v1H3v-5zm3.5-1.5l-1 1.5h13l-1-1.5h-11zM7 14h2v2H7v-2zm8 0h2v2h-2v-2z",
              },
            ].map((c) => (
              <button
                key={c.id}
                onClick={() => toggleCaroserie(c.id)}
                className={`flex flex-col items-center justify-center gap-2 p-2 rounded-xl border transition-all group h-20 ${
                  selectedCaroserii.includes(c.id)
              ...
              >
                <svg
                  className="w-8 h-8 group-hover:scale-110 transition-transform fill-current"
                  viewBox="0 0 24 24"
                >
                  <path d={c.path}></path>
                </svg>
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-400 text-sm">
              {paginatedMasini.length} vehicule afișate din {masini.length}{" "}
              disponibile
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-lg">
              ...
              >
                {hasAiTest && (
                  <option value="ai" className="bg-[#151022]">
                    AI Match %
                  </option>
                )}
                <option value="price-low" className="bg-[#151022]">
                  Preț: Crescător
                </option>
                <option value="price-high" className="bg-[#151022]">
                  Preț: Descrescător
                </option>
                <option value="newest" className="bg-[#151022]">
                  Cele mai noi
                </option>
              </select>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {paginatedMasini.map((masina) => {
            ...
            const mc =
              masina.aiMatch != null ? matchColor(masina.aiMatch) : null;
            ...
            return (
              <div
                key={masina.idMasina}
                onClick={() =>
                  !isVandut && navigate(`/client/masina/${masina.idMasina}`)
                }
                className={`group relative glass-panel rounded-2xl overflow-hidden transition-all duration-300 flex flex-col cursor-pointer ${
                  isVandut
              ...
              >
                ...
                <div className="relative h-60 overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 z-20 flex gap-2">
                    {masina.status === "Vandut" ||
                    masina.status === "Vândut" ? (
                      <span className="px-2.5 py-1 bg-gray-500/90 backdrop-blur-sm text-white text-[10px] font-bold uppercase tracking-wider rounded-md shadow-lg">
                        Vândut
                      </span>
                    ) : masina.status === "Rezervat" ? (
                      <span className="px-2.5 py-1 bg-amber-500/90 backdrop-blur-sm text-[#151022] text-[10px] font-bold uppercase tracking-wider rounded-md shadow-lg">
                        Rezervat
                      </span>
                    ) : masina.status === "În service" ? (
                      <span className="px-2.5 py-1 bg-blue-500/90 backdrop-blur-sm text-white text-[10px] font-bold uppercase tracking-wider rounded-md shadow-lg">
                        Service
                      </span>
                    ) : (
                    ...
                    {masina.combustibil === 3 && (
                      <span className="px-2.5 py-1 bg-[#2DD4BF]/90 backdrop-blur-sm text-[#151022] text-[10px] font-bold uppercase tracking-wider rounded-md shadow-lg">
                        Electric
                      </span>
                    )}
                  </div>
                  ...
                  {!isVandut && !isDirector && (
                    <div className="absolute top-4 right-4 z-20">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          toggleWishlist(masina.idMasina);
                        }}
                  ...
                  {hasAiTest ? (
                    <div className="absolute -top-6 right-5 z-20 bg-[#151022] rounded-full p-1 border border-white/10 shadow-xl">
                      <div
                        ...
                        style={{
                          background: `conic-gradient(${aiColor} ${masina.aiMatch ?? 0}%, #334155 0)`,
                        }}
                      >
                        <div className="absolute inset-1 bg-[#151022] rounded-full flex items-center justify-center">
                          <span
                            className={`text-[11px] font-bold ${mc ? mc.text : "text-slate-500"}`}
                          >
                            {aiText}
                          </span>
                        </div>
                  ...
                  <div className="flex items-end gap-3 mb-6">
                    {isVandut ? (
                      ...
                      <>
                        ...
                        {areDiscountPersonal && (
                          <span className="text-[10px] font-bold text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded mb-1.5">
                            Discount personal
                          </span>
                        )}
                      </>
                  ...
                  <div className="mt-auto">
                    {isVandut ? (
                      <div className="grid grid-cols-1 gap-3">
                        <button
                          className="py-2.5 px-4 rounded-xl bg-white/5 border border-white/10 text-white text-sm font-semibold flex items-center justify-center gap-2 cursor-not-allowed opacity-50"
                          disabled
                        >
                          Indisponibil
                        </button>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <button
                          onClick={() =>
                            navigate(
                              `/client/masina/${masina.idMasina}?testdrive=1`,
                            )
                          }
          ...
          <div className="flex items-center justify-center gap-4 mt-12 mb-8">
            ...
            <div className="flex items-center gap-2">
              {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                ...
                if (totalPages <= 5) {
                  page = i + 1;
                } else if (paginaCurenta <= 3) {
                  page = i + 1;
                } else if (paginaCurenta >= totalPages - 2) {
                  page = totalPages - 4 + i;
                } else {
                  page = paginaCurenta - 2 + i;
                }
                return (
                  <button
                    key={page}
                    onClick={() => setPaginaCurenta(page)}
                    className={`size-9 rounded-lg font-medium transition-all ${
                      paginaCurenta === page
            ...
            <button
              onClick={() =>
                setPaginaCurenta((p) => Math.min(totalPages, p + 1))
              }
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-hide">
            {recentViewed.length > 0 ? (
              ...
              <div className="flex items-center gap-3 py-4 px-2 text-gray-500 text-sm">
                ...
                <span>
                  Nu ai vizualizat încă nicio mașină. Deschide o mașină din
                  catalog pentru a o vedea aici.
                </span>
              </div>
            )}
          </div>

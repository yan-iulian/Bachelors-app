...
function WishlistMasini() {
    ...
    const loadWishlist = async () => {
        try {
            ...
            if (ids.length === 0) { setWishlist([]); setLoading(false); return; }
    ...
    const totalValue = wishlist.reduce((sum, m) => sum + (m.esteInPromotie && m.pretPromotional ? m.pretPromotional : m.pretEuro), 0);
    ...
    return (
        <main className="flex-1 max-w-[1600px] mx-auto w-full p-6 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-white flex items-center gap-3">
                        <span className="material-symbols-outlined text-[#fb7185] text-3xl">favorite</span>
                        Wishlist-ul Meu
                    </h1>
            ...
            <section className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div className="glass-panel rounded-xl p-4 flex items-center gap-3">
                    <div className="size-10 rounded-lg bg-white/5 flex items-center justify-center text-[#2DD4BF]">
                        <span className="material-symbols-outlined">smart_toy</span>
                    </div>
                    <div>
                        <p className="text-2xl font-bold text-white">
                            {wishlist.length > 0 ? Math.round(wishlist.reduce((s, m) => s + m.aiMatch, 0) / wishlist.length) : 0}%
                        </p>
                        <p className="text-xs text-slate-400">AI Match Mediu</p>
                    </div>
                </div>
            </section>
            ...
            {wishlist.length === 0 && (
                <div className="glass-panel rounded-2xl p-12 flex flex-col items-center text-center">
                    <div className="size-20 rounded-full bg-[#fb7185]/10 flex items-center justify-center mb-6">
                        <span className="material-symbols-outlined text-5xl text-[#fb7185]">heart_broken</span>
                    </div>
                    <h3 className="text-xl font-bold text-white mb-2">Wishlist-ul tău este gol</h3>
                    <p className="text-slate-400 text-sm mb-6 max-w-md">
                        Nu ai adăugat niciun vehicul în lista de favorite. Explorează catalogul nostru și salvează mașinile care te interesează!
                    </p>
                    <button
                        onClick={() => navigate('/client/catalog')}
                                ...
                                <div className="relative h-56 overflow-hidden">
                                    ...
                                    <div className="absolute top-4 left-4 z-20 flex gap-2">
                                        ...
                                        {masina.esteInPromotie && (
                                            <span className="px-2.5 py-1 bg-[#895af6]/90 backdrop-blur-sm text-white text-[10px] font-bold uppercase tracking-wider rounded-md shadow-lg">PROMO</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="absolute top-4 right-4 z-20">
                                        ...
                                        >
                                            <span className="material-symbols-outlined text-white text-lg">heart_broken</span>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="p-5 flex flex-col flex-1 relative">
                                    ...
                                    <div className="absolute -top-6 right-5 z-20 bg-[#151022] rounded-full p-1 border border-white/10 shadow-xl">
                                        <div
                                            ...
                                            style={{ background: `conic-gradient(#2DD4BF ${masina.aiMatch}%, #334155 0)` }}
                                        >
                                            <div className="absolute inset-1 bg-[#151022] rounded-full flex items-center justify-center">
                                                <span className="text-[10px] font-bold text-[#2DD4BF]">{masina.aiMatch}%</span>
                                            </div>
                                        </div>
                                    </div>

...
const getRadarScores = (m) => ({
  viteza: Math.round(
    (((m.scorViteza || 0) + (m.scorAcceleratieCuplu || 0)) / 2) * 10,
  ),
  design: Math.round(
    (((m.scorDesignInterior || 0) + (m.scorDesignExterior || 0)) / 2) * 10,
  ),
...
function PaginaMasina() {
    ...
    const fetchData = async () => {
      try {
        ...
        const similar = allCars
          ...
          .sort((a, b) => {
            const aScore =
              (a.categorieAuto === car.categorieAuto ? 10 : 0) +
              (a.marca === car.marca ? 5 : 0);
            const bScore =
              (b.categorieAuto === car.categorieAuto ? 10 : 0) +
              (b.marca === car.marca ? 5 : 0);
  ...
  useEffect(() => {
    try {
      ...
      const updated = [carId, ...stored.filter((x) => x !== carId)].slice(
        0,
        10,
      );
  ...
  const toggleWishlist = () => {
    try {
      ...
      const carId = Number(id);
      const updated = list.includes(carId)
        ? list.filter((wId) => wId !== carId)
        : [...list, carId];
  ...
  const generateFisaTehnica = () => {
    if (!masina) return "";
    const now = new Date();
    const dataGen = now.toLocaleDateString("ro-RO", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric",
    });
    const oraGen = now.toLocaleTimeString("ro-RO", {
      hour: "2-digit",
      minute: "2-digit",
    });
    const pretFinal =
...
<div class="footer">
  ...
  const handleTestDriveSubmit = async () => {
    try {
      setSubmitError(null);
      const dataProgramata =
        tdForm.data && tdForm.ora
          ? `${tdForm.data}T${tdForm.ora}:00`
          : tdForm.data;
    ...
    try {
      ...
      const pretCurent =
        masina.esteInPromotie && masina.pretPromotional
          ? masina.pretPromotional
          : masina.pretEuro;
      const discountProcent = Math.round(
        (1 - Number(discForm.pretDorit) / pretCurent) * 100,
      );
    ...
    return (
      <div className="flex flex-1 items-center justify-center min-h-[60vh]">
        <div className="glass-panel p-8 rounded-2xl text-center max-w-md">
          ...
          <p className="text-gray-400 text-sm">
            {error || "Verificați ID-ul mașinii."}
          </p>
          <button
            onClick={() => navigate("/client/catalog")}
            className="mt-4 px-6 py-2 rounded-xl bg-[#895af6] text-white font-semibold text-sm"
          >
            Înapoi la Catalog
          </button>
        </div>
  ...
  const economie =
    masina.pretEuro - (masina.pretPromotional || masina.pretEuro);
  const pretCurent =
    masina.esteInPromotie && masina.pretPromotional
      ? masina.pretPromotional
      : masina.pretEuro;
  ...
  const radarPointCoords = (() => {
    const axes = [
      masina.radarScores.viteza,
      masina.radarScores.design,
      masina.radarScores.pret,
      masina.radarScores.consum,
      masina.radarScores.confort,
    ];
    const cx = 50,
      cy = 50,
      r = 35;
  ...
  const specs = [
    {
      icon: "speed",
      label: "Rulaj",
      value: `${masina.km.toLocaleString()} km`,
    },
    {
      icon: "calendar_today",
      label: "An Fabricație",
      value: masina.anFabricatie,
    },
    {
      icon: "local_gas_station",
      label: "Combustibil",
      value: masina.combustibilText,
    },
    { icon: "settings", label: "Categorie", value: masina.categorieText },
    {
      icon: "bolt",
      label: "Viteză (scor)",
      value: `${masina.scorViteza || "—"}/10`,
    },
    {
      icon: "airline_seat_recline_extra",
      label: "Confort (scor)",
      value: `${masina.scorConfort || "—"}/10`,
    },
    {
      icon: "local_parking",
      label: "Loc Parcare",
      value: masina.locParcare || "—",
    },
    {
      icon: "swap_driving_apps_wheel",
      label: "Manevrabilitate",
      value: `${masina.scorManevrabilitate || "—"}/10`,
    },
  ];
  ...
  const statusBadge = {
    ...
    text: masina.status?.toUpperCase() || "NECUNOSCUT",
    color: "#94a3b8",
    bg: "",
  };
  ...
  return (
    <main className="relative pb-20 overflow-x-hidden">
      ...
      <section className="relative w-full h-[60vh] lg:h-[75vh] group">
        {(() => {
          const allImages =
            masina.imagini && masina.imagini.length > 0
              ...
              <div className="absolute inset-0 flex flex-col justify-between p-6 lg:p-12 max-w-[1440px] mx-auto w-full">
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  ...
                  {masina.esteInPromotie && (
                    <div className="glass-panel px-4 py-1.5 rounded-full flex items-center gap-2 border-l-4 border-l-[#fb7185] bg-[#fb7185]/10">
                      <span className="text-xs font-bold tracking-wider text-[#fb7185]">
                        PROMOȚIE -
                        {Math.round(
                          (1 - masina.pretPromotional / masina.pretEuro) * 100,
                        )}
                        %
                      </span>
                    </div>
                ...
                {allImages.length > 1 && (
                  <div className="absolute inset-y-0 left-4 right-4 flex items-center justify-between pointer-events-none">
                    ...
                    <button
                      onClick={() =>
                        setCurrentThumb((currentThumb + 1) % allImages.length)
                      }
                      className="pointer-events-auto size-12 rounded-full glass-panel flex items-center justify-center hover:bg-white/10 transition-all transform hover:scale-105"
                    >
                      <span className="material-symbols-outlined">
                  ...
                  <div className="hidden lg:flex gap-3 overflow-x-auto pb-2 scrollbar-hide max-w-2xl">
                    {allImages.slice(0, 5).map((img, i) => (
                      <div
                        ...
                        className={`w-24 h-16 rounded-lg bg-cover bg-center cursor-pointer transition-all border-2 ${
                          currentThumb === i
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2 mb-1">
                  <div className="flex text-[#D4AF37] text-sm">
                    {Array.from({ length: 5 }, (_, i) => (
                      ...
                      >
                        {i < Math.floor(masina.aiMatch / 20)
                          ? "star"
                          : i < Math.ceil(masina.aiMatch / 20)
                            ? "star_half"
                            : "star_border"}
                      </span>
                    ))}
              ...
              <p className="text-xl text-[#895af6] font-medium">
                {masina.combustibilText} • {masina.categorieText} •{" "}
                {masina.anFabricatie}
              </p>
            </div>
            ...
            <div className="glass-panel p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 border-l-4 border-l-[#D4AF37]">
              <div className="flex flex-col">
                <span className="text-white/50 text-sm uppercase tracking-wider font-semibold mb-1">
                  Preț Final
                </span>
                <div className="flex items-baseline gap-3">
                  <span className="text-4xl lg:text-5xl font-bold text-[#D4AF37]">
                    €{pretCurent.toLocaleString()}
                  </span>
                  {masina.esteInPromotie &&
                    masina.pretPromotional &&
                    masina.pretPromotional < masina.pretEuro && (
                      <span className="text-lg text-white/30 line-through decoration-white/30">
                ...
                {economie > 0 && (
                  <div className="mt-2 inline-flex">
                    <span className="bg-[#D4AF37]/20 text-[#D4AF37] text-xs font-bold px-2 py-1 rounded">
                      Economisești €{economie.toLocaleString()}
                    </span>
                  </div>
                )}
              </div>
              {!isDirector && (
                <div className="flex flex-col gap-3 min-w-[200px]">
                  <button
                    onClick={() =>
                      navigate("/client/simulare-credit", {
                        state: {
                          pretMasina: pretCurent,
                          numeMasina: `${masina.marca} ${masina.model}`,
                        },
                      })
                    }
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {specs.map((spec, i) => (
                  <div
                    key={i}
                    className="glass-panel p-4 rounded-xl flex flex-col items-start gap-3 hover:bg-white/5 transition-colors group cursor-default"
                  >
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
                ...
                <div className="flex flex-col items-center gap-6">
                  ...
                  <div className="relative size-32 flex items-center justify-center">
                    ...
                    <div className="absolute flex flex-col items-center">
                      ...
                      <span className="text-[10px] uppercase text-[#2DD4BF] tracking-wider font-semibold">
                        Match
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="w-full h-40 relative flex items-center justify-center">
                    ...
                    >
                      <polygon
                        points={bgPentagonPoints}
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="1"
                      ></polygon>
                      <polygon
                        points={innerPentagonPoints}
                        fill="none"
                        stroke="currentColor"
                        strokeWidth="0.5"
                      ></polygon>
                    ...
                    {/* Labels */}
                    <div className="absolute top-0 text-[10px] text-white/60">
                      Viteză
                    </div>
                    <div className="absolute right-0 top-1/3 text-[10px] text-white/60">
                      Design
                    </div>
                    <div className="absolute bottom-0 right-8 text-[10px] text-white/60">
                      Preț
                    </div>
                    <div className="absolute bottom-0 left-8 text-[10px] text-white/60">
                      Consum
                    </div>
                    <div className="absolute left-0 top-1/3 text-[10px] text-white/60">
                      Confort
                    </div>
                  </div>
                  ...
                  <a
                    ...
                    className="text-xs text-[#2DD4BF] underline decoration-[#2DD4BF]/50 hover:text-white transition-colors cursor-pointer"
                  >
            ...
            <div className="flex flex-col gap-3 sticky top-24">
              {isDirector ? (
                ...
                <div className="glass-panel rounded-2xl p-5 border border-teal-500/20 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        Vizualizezi pagina așa cum o vede un client
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => navigate("/director/masini")}
                    className="w-full py-3 rounded-xl bg-teal-500/10 hover:bg-teal-500/20 text-teal-400 font-semibold text-sm transition-colors flex items-center justify-center gap-2 border border-teal-500/20"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      arrow_back
                    </span>
                    Înapoi la Gestiune Mașini
                  </button>
                  <button
                    onClick={handleDownloadPDF}
                    className="w-full py-3 rounded-xl bg-white/5 hover:bg-white/10 text-white font-semibold text-sm transition-colors flex items-center justify-center gap-2 border border-white/10"
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      picture_as_pdf
                    </span>
                ...
                <>
                  {isIndisponibil && (
                    ...
                    >
                      <span className="material-symbols-outlined text-[18px]">
                        {masina.status === "Rezervat" ? "lock" : "block"}
                      </span>
                      {masina.status === "Rezervat"
                        ? "Mașină Rezervată — Tranzacție în curs"
                        : "Mașină Vândută"}
                    </div>
                  )}
                  <button
                    onClick={() => setShowTestDrive(true)}
                    disabled={isIndisponibil}
                    className={`w-full py-4 rounded-xl text-white font-bold text-lg shadow-lg transition-all flex items-center justify-center gap-3 ${
                      isIndisponibil
                        ? "bg-slate-700 opacity-40 cursor-not-allowed shadow-none"
                        : "bg-gradient-to-r from-[#895af6] to-[#7040d6] hover:brightness-110 shadow-[#895af6]/25"
                    }`}
                  >
                    <span className="material-symbols-outlined">
                      directions_car
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className={`py-3 rounded-xl font-semibold text-sm transition-colors flex items-center justify-center gap-2 ${
                        inWishlist
                    ...
                    <button
                      onClick={handleDownloadPDF}
                      className="py-3 rounded-xl bg-[#2e2249] hover:bg-[#3d2e5e] text-white font-semibold text-sm transition-colors flex items-center justify-center gap-2"
                    >
                      <span className="material-symbols-outlined text-[18px]">
                        picture_as_pdf
                      </span>
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2 ${
                      isIndisponibil
                        ? "border border-slate-600 text-slate-600 opacity-40 cursor-not-allowed"
                        : "border border-[#D4AF37]/30 hover:bg-[#D4AF37]/10 text-[#D4AF37]"
                    }`}
                  >
                    <span className="material-symbols-outlined text-[18px]">
                      sell
                  ...
                  <button
                    onClick={() => setShowCumpara(true)}
                    disabled={isIndisponibil}
                    className={`w-full py-4 rounded-xl text-white font-bold text-lg shadow-lg transition-all flex items-center justify-center gap-3 ${
                      isIndisponibil
                        ? "bg-slate-700 opacity-40 cursor-not-allowed shadow-none"
                        : "bg-gradient-to-r from-[#10b981] to-[#059669] hover:brightness-110 shadow-[#10b981]/25"
                    }`}
                  >
                    <span className="material-symbols-outlined">
                      shopping_cart
              ...
              {!isDirector && (
                <div className="glass-panel p-4 rounded-xl mt-4 flex items-center gap-4">
                  <div
                    ...
                    style={{
                      backgroundImage: `url('https://placehold.co/100x100/2e2249/895af6?text=AY')`,
                    }}
                  ></div>
      ...
      {!isDirector && (
        <section className="max-w-[1440px] mx-auto px-4 lg:px-12 mt-16 lg:mt-24">
          ...
          <div
            className="flex overflow-x-auto gap-4 pb-4 snap-x snap-mandatory -mx-4 px-4 lg:px-0"
            style={{
              scrollbarWidth: "thin",
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <p className="text-[11px] text-white/50 mb-2">
                    {car.anFabricatie} • {car.km.toLocaleString()} km
                  </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            ...
            onClick={() => {
              ...
              setTdForm({ data: "", ora: "", telefon: "", note: "" });
            }}
          ></div>
            ...
            {tdSubmitted ? (
              <div className="p-10 flex flex-col items-center gap-5 text-center animate-[scaleIn_0.3s_ease-out]">
                ...
                <p className="text-slate-400 text-sm max-w-sm">
                  Cererea ta de test drive a fost trimisă către Director. Vei
                  primi o notificare când va fi aprobată.
                </p>
                <button
                  onClick={() => {
                    setShowTestDrive(false);
                  ...
                  className="px-8 py-3 rounded-xl bg-[#895af6] text-white font-bold text-sm hover:bg-[#7a4ae0] transition"
                >
                  Închide
                </button>
              </div>
              ...
              <>
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        value={tdForm.data}
                        onChange={(e) =>
                          setTdForm((f) => ({ ...f, data: e.target.value }))
                        }
                        min="2026-02-15"
                    ...
                    <div>
                      ...
                      <select
                        value={tdForm.ora}
                        onChange={(e) =>
                          setTdForm((f) => ({ ...f, ora: e.target.value }))
                        }
                        className="w-full p-3 text-sm text-white bg-white/5 border border-white/10 rounded-xl focus:ring-[#895af6] focus:border-[#895af6] outline-none transition"
                      >
                        <option value="" className="bg-[#1e202d]">
                          Selectează
                        </option>
                        {[
                          "09:00",
                          ...
                          <option key={h} value={h} className="bg-[#1e202d]">
                            {h}
                          </option>
                        ))}
                      </select>
                  ...
                  <div>
                    ...
                    <input
                      type="tel"
                      value={tdForm.telefon}
                      onChange={(e) =>
                        setTdForm((f) => ({ ...f, telefon: e.target.value }))
                      }
                  ...
                  <div>
                    ...
                    <textarea
                      value={tdForm.note}
                      onChange={(e) =>
                        setTdForm((f) => ({ ...f, note: e.target.value }))
                      }
                      rows={3}
                  ...
                  <div className="bg-amber-500/5 rounded-xl p-3 border border-amber-500/10 flex items-start gap-2">
                    ...
                    <p className="text-xs text-amber-400/80">
                      Se percepe o taxă de test drive de <strong>50 €</strong>,
                      achitabilă la sediu în ziua programării.
                    </p>
                  </div>
                </div>
                <div className="flex items-center justify-between p-6 border-t border-white/10">
                  ...
                  <button
                    ...
                    disabled={!tdForm.data || !tdForm.ora || !tdForm.telefon}
          ...
          <div className="relative glass-panel rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl">
            {discSubmitted ? (
              <div className="p-10 flex flex-col items-center gap-5 text-center animate-[scaleIn_0.3s_ease-out]">
                ...
                <p className="text-slate-400 text-sm max-w-sm">
                  Cererea ta de discount a fost trimisă către Director. Vei fi
                  notificat cu decizia acestuia.
                </p>
                <button
                  onClick={() => {
                    setShowDiscount(false);
                  ...
                  className="px-8 py-3 rounded-xl bg-[#D4AF37] text-[#151022] font-bold text-sm hover:bg-[#c9a432] transition"
                >
                  Închide
                </button>
              </div>
                ...
                <div className="p-6 space-y-5">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        type="number"
                        value={discForm.pretDorit}
                        onChange={(e) =>
                          setDiscForm((f) => ({
                            ...f,
                            pretDorit: e.target.value,
                          }))
                        }
                        placeholder={`Max ${pretCurent.toLocaleString()}`}
                    ...
                    {discForm.pretDorit &&
                      Number(discForm.pretDorit) > 0 &&
                      Number(discForm.pretDorit) < pretCurent && (
                        <p className="text-xs mt-2 flex items-center gap-1">
                          ...
                          <span className="text-[#D4AF37]">
                            Discount solicitat: €
                            {(
                              pretCurent - Number(discForm.pretDorit)
                            ).toLocaleString()}{" "}
                            (
                            {(
                              ((pretCurent - Number(discForm.pretDorit)) /
                  ...
                  <div>
                    ...
                    <textarea
                      value={discForm.motiv}
                      onChange={(e) =>
                        setDiscForm((f) => ({ ...f, motiv: e.target.value }))
                      }
                      rows={3}
                  ...
                  <div className="bg-white/[0.02] rounded-xl p-3 border border-white/5 flex items-start gap-2">
                    ...
                    <p className="text-xs text-slate-500">
                      Directorul va analiza cererea și va răspunde printr-o
                      notificare. Prețul final este la latitudinea acestuia.
                    </p>
                  </div>
                </div>
                <div className="flex items-center justify-between p-6 border-t border-white/10">
                  ...
                  <button
                    ...
                    disabled={!discForm.pretDorit || !discForm.motiv.trim()}
      ...
      {showCumpara && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-black/60 backdrop-blur-sm"
            onClick={() => {
              setShowCumpara(false);
              setCumparaSubmitted(false);
              setTipPlata("Cash");
              setCumparaStep(1);
            }}
          ></div>
          <div className="relative glass-panel rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl">
            {cumparaSubmitted ? (
              <div className="p-10 flex flex-col items-center gap-5 text-center animate-[scaleIn_0.3s_ease-out]">
                ...
                <h2 className="text-2xl font-bold text-white">
                  Cerere de Achiziție Trimisă!
                </h2>
                <p className="text-slate-400 text-sm max-w-sm">
                  Mașina a fost rezervată în numele tău. Directorul va analiza
                  cererea și va aproba tranzacția.
                </p>
              ...
              <div className="p-8 flex flex-col items-center gap-5 text-center">
                ...
                <p className="text-xs text-slate-400">
                  După confirmare, mașina va fi{" "}
                  <strong className="text-amber-400">rezervată</strong> și nu va
                  mai fi disponibilă altor clienți până la verdictul
                  Directorului.
                </p>
                {submitError && (
            ...
            <div className="flex items-center justify-between p-5 border-b border-white/10 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {masina.marca} {masina.model} · {masina.anFabricatie}
                  </p>
                </div>
              </div>
              <button

...
function GestiunePiese() {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                if (selectedPiesa) {
                    ...
                    if (updated) setSelectedPiesa({ ...updated, nrFurnizori: selectedPiesa.nrFurnizori });
                }
            }, 1500);
        } catch (e) {
    ...
    const pieseFiltrate = piese
        .filter(p => {
            if (categorieActiva !== 'Toate' && p.categorie !== categorieActiva) return false;
            ...
            if (search) {
                ...
                return p.denumire.toLowerCase().includes(q)
    ...
    const stocBadge = (stoc) => {
        if (stoc === 0) return { label: 'Lipsă', cls: 'bg-red-500/10 text-red-400 border-red-500/20' };
        if (stoc <= 3) return { label: 'Stoc Scăzut', cls: 'bg-amber-500/10 text-amber-400 border-amber-500/20' };
        return { label: 'In Stoc', cls: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' };
    };
    ...
    return (
        <div className="min-h-screen bg-[#151022] pt-20 px-6 pb-8">
            ...
            <div className="max-w-7xl mx-auto mb-8">
                ...
                <p className="text-gray-400 text-sm">Inventar piese, stoc și furnizori</p>
            </div>

            {/* KPI Cards */}
            <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {[
                    { label: 'Total Piese', value: totalPiese, icon: 'inventory_2', color: 'text-[#895af6]' },
                    { label: 'Valoare Stoc', value: `${(valoareTotala / 1000).toFixed(0)}k RON`, icon: 'payments', color: 'text-emerald-400' },
                    { label: 'Stoc Zero', value: stocZero, icon: 'error', color: 'text-red-400' },
                    { label: 'Stoc Scăzut', value: stocScazut, icon: 'warning', color: 'text-amber-400' },
                ].map((k, i) => (
            ...
            <div className="max-w-7xl mx-auto glass-panel rounded-2xl p-4 mb-6 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center">
                    ...
                    >
                        <option value="denumire" className="bg-[#1a1333] text-white">Nume A-Z</option>
                        <option value="pret-asc" className="bg-[#1a1333] text-white">Preț ↑</option>
                        <option value="pret-desc" className="bg-[#1a1333] text-white">Preț ↓</option>
                        <option value="stoc" className="bg-[#1a1333] text-white">Stoc ↑</option>
                    </select>
                </div>

                {/* Row 2: Categorie filters */}
                <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-500 font-medium whitespace-nowrap">Categorie:</span>
                    <div className="flex gap-1.5 flex-wrap">
                        {['Toate', ...categorii].map(c => (
                            <button
                                key={c}
                                onClick={() => setCategorieActiva(c)}
                                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all whitespace-nowrap ${categorieActiva === c
                ...
                {/* Row 3: Stoc filters */}
                <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-500 font-medium whitespace-nowrap">Stoc:</span>
                    <div className="flex gap-1.5 flex-wrap">
                        {['Toate', 'In Stoc', 'Stoc Scăzut', 'Lipsă'].map(s => (
                            <button
                                key={s}
                                onClick={() => setStocFilter(s)}
                                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all whitespace-nowrap ${stocFilter === s
                                    ? 'bg-[#895af6] text-white border-[#895af6]'
                                    : 'bg-white/5 text-gray-400 border-white/10 hover:border-white/30 hover:text-white'
                                }`}
                            >
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 glass-panel rounded-2xl overflow-hidden lg:max-h-[calc(100vh-20rem)]">
                    <div className="overflow-x-auto overflow-y-auto lg:max-h-[calc(100vh-20rem)]" style={{ scrollbarWidth: 'thin', scrollbarColor: '#895af6 transparent' }}>
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="px-6 py-4">
                                                <span className="font-medium text-white">{p.denumire}</span>
                                            </td>
                                            <td className="px-6 py-4 text-slate-400">{p.categorie}</td>
                                            <td className="px-6 py-4 text-slate-400 text-xs">{p.compatibilitate}</td>
                                            ...
                                            <td className="px-6 py-4 text-right font-mono text-white">{p.pret.toLocaleString()} RON</td>
                                        </tr>
                                    );
                                })}
                ...
                {/* Detail Panel */}
                <div className="glass-panel rounded-2xl p-6 flex flex-col lg:sticky lg:top-24 lg:self-start lg:max-h-[calc(100vh-20rem)] lg:overflow-y-auto" style={{ scrollbarWidth: 'thin', scrollbarColor: '#895af6 transparent' }}>
                    {selectedPiesa ? (
                        <>
                            <div className="flex items-center gap-3 mb-6">
                                <div className="size-12 rounded-xl bg-[#895af6]/20 flex items-center justify-center">
                                    <span className="material-symbols-outlined text-[#895af6] text-2xl">build</span>
                                </div>
                                <div>
                                    <h3 className="text-lg font-bold text-white">{selectedPiesa.denumire}</h3>
                                    <p className="text-xs text-slate-400">{selectedPiesa.categorie}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4 flex-1">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-white/[0.03] rounded-xl p-4">
                                        ...
                                        <p className="text-lg font-bold text-white font-mono">{selectedPiesa.pret.toLocaleString()} RON</p>
                                    </div>
                                    <div className="bg-white/[0.03] rounded-xl p-4">
                                        <p className="text-xs text-slate-400 mb-1">Stoc Total</p>
                                        <p className={`text-lg font-bold ${furnizoriPiesa.reduce((s, p) => s + p.stoc, 0) === 0 ? 'text-red-400' : furnizoriPiesa.reduce((s, p) => s + p.stoc, 0) <= 3 ? 'text-amber-400' : 'text-emerald-400'}`}>
                                ...
                                <div className="bg-white/[0.03] rounded-xl p-4">
                                    ...
                                    <p className="text-sm text-white font-medium">{selectedPiesa.compatibilitate}</p>
                                </div>
                                ...
                                <div>
                                    <p className="text-xs text-slate-400 mb-2 flex items-center gap-1">
                                        ...
                                        {furnizoriPiesa.length} {furnizoriPiesa.length === 1 ? 'furnizor disponibil' : 'furnizori disponibili'}
                                    </p>
                                    <div className="space-y-2">
                                        {furnizoriPiesa.map(f => (
                                            <div key={f.idPiesa} className="bg-white/[0.03] rounded-xl p-3 flex items-center gap-3 border border-white/5">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-sm font-medium text-white truncate">{f.furnizorNume}</p>
                                                    <div className="flex items-center gap-3 text-xs text-slate-400">
                                                        <span className="font-mono">{f.pret.toLocaleString()} RON</span>
                                                        <span>•</span>
                                                        <span className={f.stoc === 0 ? 'text-red-400' : f.stoc <= 3 ? 'text-amber-400' : 'text-emerald-400'}>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center">
                            <span className="material-symbols-outlined text-5xl text-slate-600 mb-4">touch_app</span>
                            <p className="text-slate-400 text-sm">Selectează o piesă din tabel pentru a vedea detaliile</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {showComandaModal && selectedPiesa && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowComandaModal(false)}></div>
                    <div className="relative glass-panel rounded-2xl w-full max-w-lg p-6 shadow-2xl border border-white/10">
                        {/* Close */}
                        <button onClick={() => setShowComandaModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white transition">
                        ...
                        {comandaTrimisa ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-8">
                                ...
                                <h3 className="text-xl font-bold text-white mb-2">Comandă Trimisă!</h3>
                                <p className="text-sm text-slate-400 text-center">
                                    {comandaCantitate}x {selectedPiesa.denumire} de la <span className="text-white font-medium">{comandaFurnizor?.furnizorNume}</span>
                                </p>
                            </div>
                        ) : (
                            <>
                                ...
                                <div className="mb-6">
                                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                                        <span className="material-symbols-outlined text-[#895af6]">shopping_cart</span>
                                        Comandă Piesă
                                    </h3>
                                    <p className="text-sm text-slate-400 mt-1">{selectedPiesa.denumire} — {selectedPiesa.categorie}</p>
                                </div>

                                {/* Cantitate */}
                                <div className="mb-6">
                                    <label className="text-sm font-semibold text-slate-300 mb-2 block">Cantitate</label>
                                    <div className="flex items-center gap-3">
                                        <button
                                            onClick={() => setComandaCantitate(prev => Math.max(1, prev - 1))}
                                        ...
                                        <span className="text-xl font-bold text-white w-12 text-center">{comandaCantitate}</span>
                                        <button
                                        ...
                                        {furnizoriPiesa.map(f => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full text-left rounded-xl p-4 border transition-all ${isChosen
                                                        ? 'bg-[#895af6]/15 border-[#895af6] shadow-[0_0_15px_rgba(137,90,246,0.15)]'
                                                        : 'bg-white/[0.03] border-white/10 hover:bg-white/[0.06] hover:border-white/20'
                                                    }`}
                                                >
                                                    <div className="flex items-center gap-3">
                                                        <div className={`size-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${isChosen ? 'bg-[#895af6] text-white' : 'bg-white text-[#151022]'}`}>
                                                            {f.furnizorNume.substring(0, 2).toUpperCase()}
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-sm font-bold text-white">{f.furnizorNume}</p>
                                                            <p className="text-xs text-slate-400">{f.furnizorTelefon}</p>
                                                        </div>
                                                        <div className="text-right shrink-0">
                                                            <p className="text-sm font-bold text-white font-mono">{f.pret.toLocaleString()} RON</p>
                                                            <div className="flex items-center gap-2 justify-end text-xs mt-0.5">
                                                                <span className={f.stoc === 0 ? 'text-red-400' : f.stoc <= 3 ? 'text-amber-400' : 'text-emerald-400'}>
                                                                    {f.stoc === 0 ? 'Lipsă stoc' : `${f.stoc} în stoc`}
                                                                </span>
                                                                <span className="text-slate-500">•</span>
                                                                <span className="text-slate-400 flex items-center gap-0.5">
                                                                    <span className="material-symbols-outlined text-[12px]">local_shipping</span> {f.timpLivrareOre}h
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                ...
                                {comandaFurnizor && (
                                    <div className="bg-white/[0.03] rounded-xl p-4 mb-4 flex items-center justify-between">
                                        <span className="text-sm text-slate-300">Total comandă:</span>
                                        <span className="text-xl font-bold text-[#895af6] font-mono">
                                            {(comandaFurnizor.pret * comandaCantitate).toLocaleString()} RON
                                        </span>
                                    </div>
                                )}
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 px-4 py-3 rounded-xl text-white font-bold text-sm transition flex items-center justify-center gap-2 ${comandaFurnizor

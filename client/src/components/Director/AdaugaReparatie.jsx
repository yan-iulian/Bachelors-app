...
const categorieMap = { 0: 'Sedan', 1: 'Hatchback', 2: 'Coupe', 3: 'Break', 4: 'SUV', 5: 'Cabrio' };
const prioritateMap = { HIGH: 'Urgent', MEDIUM: 'Mediu', LOW: 'Scăzut' };

const statusLabels = {
    0: 'În Așteptare', 1: 'În Lucru', 2: 'Finalizată',
};
const statusColors = {
...
const emptyForm = {
    ...
    categorieAuto: 4, nrInmatriculare: '', vin: '',
    descriereProblema: '', prioritate: 'MEDIUM', pretAchizitie: 0,
    idMecanic: 1,
};
...
function AdaugaReparatie() {
    ...
    const fetchData = async () => {
        try {
            ...
            setMecaniciDisponibili(mecanici.map(m => ({ id: m.idUtilizator, nume: `${m.nume} ${m.prenume}`, specializare: '' })));
            setIstoric(reparatiiData.map(r => ({
                id: r.idReparatie,
                ...
                dataTrimisa: r.dataInceput ? new Date(r.dataInceput).toISOString().split('T')[0] : '—',
                prioritate: 'MEDIUM',
                pretAchizitie: r.cost || 0,
                status: r.statusReparatie,
            })));
            if (mecanici.length > 0) {
                setFormData(prev => ({ ...prev, idMecanic: mecanici[0].idUtilizator }));
            }
    ...
    const handleSubmit = async () => {
        if (!formData.marca.trim() || !formData.model.trim() || !formData.descriereProblema.trim()) return;
        setSuccessAnim(true);
        try {
            // Creează mașina cu status 'În service'
            const masinaNoua = await apiPost('/api/masini', {
                marca: formData.marca, model: formData.model,
                anFabricatie: formData.anFabricatie, km: formData.km,
                combustibil: formData.combustibil, pretEuro: formData.pretAchizitie,
                status: 'În service', categorieAuto: formData.categorieAuto,
                locParcare: formData.nrInmatriculare || 'N/A',
            });
            // Creează reparația
            await apiPost('/api/reparatii', {
                idMasina: masinaNoua.idMasina,
                idMecanic: formData.idMecanic,
                descriereProblema: formData.descriereProblema,
                cost: formData.pretAchizitie,
                statusReparatie: 0,
            });
            const mecanic = mecaniciDisponibili.find(m => m.id === Number(formData.idMecanic));
            setTimeout(async () => {
                await fetchData();
    ...
    if (loading) return <div className="flex-1 flex items-center justify-center"><div className="text-slate-400 text-lg">Se încarcă...</div></div>;
    ...
    return (
        <main className="flex-1 max-w-[1600px] mx-auto w-full p-6 space-y-6" style={{ paddingTop: '6rem' }}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-white flex items-center gap-3">
                        <span className="material-symbols-outlined text-[#895af6] text-3xl">add_circle</span>
                        Adaugă Mașină la Reparat
                    </h1>
                    <p className="text-slate-400 text-sm mt-1">Trimite mașini către mecanic pentru evaluare și reparație</p>
                </div>
                <button
                    onClick={handleOpen}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {[
                    { label: 'Total Trimise', val: stats.total, icon: 'assignment', color: 'text-[#895af6]', bg: 'bg-[#895af6]/10' },
                    { label: 'Așteaptă Estimare', val: stats.trimise, icon: 'schedule', color: 'text-amber-400', bg: 'bg-amber-500/10' },
                    { label: 'Estimări Primite', val: stats.estimari, icon: 'rate_review', color: 'text-[#895af6]', bg: 'bg-[#895af6]/10' },
                    { label: 'Aprobate / Lucru', val: stats.aprobate, icon: 'check_circle', color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
                ].map(k => (
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="bg-white/[0.03] border-b border-white/10">
                            <tr>
                                ...
                                <th className="px-5 py-3 text-xs font-bold text-slate-400 uppercase">Nr. Înmtr.</th>
                                ...
                                <th className="px-5 py-3 text-xs font-bold text-slate-400 uppercase">Preț Achiziție</th>
                                <th className="px-5 py-3 text-xs font-bold text-slate-400 uppercase">Prioritate</th>
                                <th className="px-5 py-3 text-xs font-bold text-slate-400 uppercase">Status</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {istoric.map(r => {
                                const pr = r.prioritate === 'HIGH' ? 'text-red-400 bg-red-500/10 border-red-500/20'
                                    : r.prioritate === 'MEDIUM' ? 'text-amber-400 bg-amber-500/10 border-amber-500/20'
                                    : 'text-slate-400 bg-slate-500/10 border-slate-500/20';
                                return (
                                    <tr key={r.id} className="hover:bg-white/[0.02] transition">
                                        <td className="px-5 py-4 text-white font-medium">{r.masina}</td>
                                        <td className="px-5 py-4 text-slate-400 font-mono text-xs">{r.nrInmatriculare}</td>
                                        <td className="px-5 py-4 text-slate-300">{r.mecanic}</td>
                                        <td className="px-5 py-4 text-slate-400">{r.dataTrimisa}</td>
                                        <td className="px-5 py-4 text-slate-300 font-mono">{r.pretAchizitie?.toLocaleString()} €</td>
                                        <td className="px-5 py-4">
                                            <span className={`text-[10px] font-bold px-2 py-1 rounded-full border ${pr}`}>
                                                {prioritateMap[r.prioritate]}
                                            </span>
                                        </td>
                                        <td className="px-5 py-4">
                                            <span className={`text-[10px] font-bold px-2 py-1 rounded-full border ${statusColors[r.status]}`}>
            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => !successAnim && setShowModal(false)}></div>
                    <div className="relative glass-panel rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl">
                        ...
                        {successAnim && (
                            <div className="absolute inset-0 z-30 bg-[#151022]/95 flex flex-col items-center justify-center rounded-2xl">
                                <div className="size-24 bg-emerald-500/20 rounded-full flex items-center justify-center mb-4 animate-[scaleIn_0.4s_ease-out]">
                                    <span className="material-symbols-outlined text-emerald-400 text-6xl">check_circle</span>
                                </div>
                                <p className="text-xl font-bold text-white mb-1">Reparație Trimisă!</p>
                                <p className="text-sm text-slate-400">Mecanicul a fost notificat — așteaptă estimarea costurilor</p>
                            </div>
                        )}
                        ...
                        <div className="flex items-center justify-between p-6 border-b border-white/10">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold text-white">Adaugă Mașină la Reparat</h2>
                                    <p className="text-xs text-slate-400">Completează datele mașinii — mecanicul va estima costul reparației</p>
                                </div>
                            </div>
                            <button onClick={() => setShowModal(false)} className="size-9 rounded-full hover:bg-white/10 flex items-center justify-center transition">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div>
                                <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider mb-4 flex items-center gap-2">
                                    <span className="material-symbols-outlined text-[16px] text-[#895af6]">directions_car</span> Date Mașină
                                </h3>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {/* Marca */}
                                    <div>
                                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">Marcă *</label>
                                        <input
                                            type="text"
                                    ...
                                    {/* An Fabricație */}
                                    <div>
                                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">An Fabricație</label>
                                        <input
                                            type="number"
                                            value={formData.anFabricatie}
                                            onChange={e => handleChange('anFabricatie', Number(e.target.value))}
                                            min={2000} max={2026}
                                    ...
                                    {/* Kilometraj */}
                                    <div>
                                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">Kilometraj (km)</label>
                                        <input
                                            type="number"
                                            value={formData.km}
                                    ...
                                    {/* Combustibil */}
                                    <div>
                                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">Combustibil</label>
                                        <select
                                            value={formData.combustibil}
                                            onChange={e => handleChange('combustibil', Number(e.target.value))}
                                        ...
                                        >
                                            {Object.entries(combustibilMap).map(([k, v]) => (
                                                <option key={k} value={k} className="bg-[#1a1333]">{v}</option>
                                            ))}
                                        </select>
                                    </div>
                                    {/* Caroserie */}
                                    <div>
                                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">Tip Caroserie</label>
                                        <select
                                            value={formData.categorieAuto}
                                            onChange={e => handleChange('categorieAuto', Number(e.target.value))}
                                            className="w-full p-3 text-sm text-white bg-white/5 border border-white/10 rounded-xl focus:ring-[#895af6] focus:border-[#895af6] outline-none transition appearance-none"
                                        >
                                    ...
                                    {/* Nr Înmatriculare */}
                                    <div>
                                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">Nr. Înmatriculare</label>
                                        <input
                                            type="text"
                                            value={formData.nrInmatriculare}
                                            onChange={e => handleChange('nrInmatriculare', e.target.value.toUpperCase())}
                                    ...
                                    {/* VIN */}
                                    <div>
                                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">VIN (Serie Șasiu)</label>
                                        <input
                                            type="text"
                                            value={formData.vin}
                            ...
                            <div>
                                <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider mb-4 flex items-center gap-2">
                                    <span className="material-symbols-outlined text-[16px] text-[#895af6]">build</span> Detalii Reparație
                                </h3>
                                <div className="space-y-4">
                                    {/* Descriere Problemă */}
                                    <div>
                                        <label className="block text-xs text-slate-400 mb-1.5 font-medium">Descriere Problemă *</label>
                                        <textarea
                                            value={formData.descriereProblema}
                                            onChange={e => handleChange('descriereProblema', e.target.value)}
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        {/* Prioritate */}
                                        <div>
                                            <label className="block text-xs text-slate-400 mb-1.5 font-medium">Prioritate</label>
                                            <div className="flex gap-2">
                                                {[
                                                    { val: 'HIGH', label: 'Urgent', color: 'bg-red-500', hoverBg: 'hover:bg-red-500/20' },
                                                    { val: 'MEDIUM', label: 'Mediu', color: 'bg-amber-500', hoverBg: 'hover:bg-amber-500/20' },
                                                    { val: 'LOW', label: 'Scăzut', color: 'bg-slate-500', hoverBg: 'hover:bg-slate-500/20' },
                                                ].map(p => (
                                                    <button
                                                        key={p.val}
                                                        type="button"
                                                        onClick={() => handleChange('prioritate', p.val)}
                                                        className={`flex-1 text-xs font-bold py-2.5 rounded-lg transition border ${formData.prioritate === p.val
                                        ...
                                        {/* Preț Achiziție */}
                                        <div>
                                            <label className="block text-xs text-slate-400 mb-1.5 font-medium">Preț Achiziție (€) *</label>
                                            <input
                                                type="number"
                                                value={formData.pretAchizitie}
                                                onChange={e => handleChange('pretAchizitie', Number(e.target.value))}
                                                min={0}
                                                className="w-full p-3 text-sm text-white bg-white/5 border border-white/10 rounded-xl focus:ring-[#895af6] focus:border-[#895af6] outline-none transition"
                                            />
                                        </div>
                                        ...
                                        {/* Mecanic */}
                                        <div>
                                            <label className="block text-xs text-slate-400 mb-1.5 font-medium">Asignează Mecanic</label>
                                            <select
                                                value={formData.idMecanic}
                                                onChange={e => handleChange('idMecanic', Number(e.target.value))}
                                                className="w-full p-3 text-sm text-white bg-white/5 border border-white/10 rounded-xl focus:ring-[#895af6] focus:border-[#895af6] outline-none transition appearance-none"
                                            >
                                                {mecaniciDisponibili.map(m => (
                                                    <option key={m.id} value={m.id} className="bg-[#1a1333]">{m.nume} — {m.specializare}</option>
                                                ))}
                                            </select>
                                        </div>
                                    </div>
                        ...
                        <div className="flex items-center justify-between p-6 border-t border-white/10">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    ...
                                    disabled={!formData.marca.trim() || !formData.model.trim() || !formData.descriereProblema.trim() || !formData.pretAchizitie}
